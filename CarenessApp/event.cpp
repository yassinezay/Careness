...
{
    model->setQuery("SELECT * FROM EVENT order by nbrinvit DESC");
...
{
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM EVENT where nbrinvit is null");
...
int Event::compteur1()
{
    ...
    if (query.exec() && query.next()) {
        int numRows = query.value(0).toInt()-compteur();
        return numRows;
    }else
    {
        return  0;
...
QSqlQueryModel * Event::recherche2(QString a)
{
QSqlQueryModel * model= new QSqlQueryModel();
    model->setQuery("select * from event WHERE id LIKE '%"+a+"%' OR nom LIKE '%"+a+"%' OR type LIKE '%"+a+"%'  OR endroit LIKE '%"+a+"%' OR nbrinvit LIKE '%"+a+"%' OR date_event LIKE '%"+a+"%'" );
