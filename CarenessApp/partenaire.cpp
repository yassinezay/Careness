...
model->setHeaderData(2, Qt::Horizontal, QObject::tr("Num_telephone"));
...
int partenaire::compteur()
{
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM partenaire where type is null");
...
int partenaire::compteur1()
{
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM partenaire");
    if (query.exec() && query.next()) {
